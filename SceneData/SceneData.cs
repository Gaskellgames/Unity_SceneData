...
namespace Gaskellgames
{
    ...
    public class SceneData : ISerializationCallbackReceiver, IComparable<SceneData>
    {
...
#if UNITY_EDITOR
        #region EditorOnly Getter / Setter
        ...
        {
            if (0 <= buildIndex && buildIndex < SceneManager.sceneCountInBuildSettings)
            {
        ...
        {
            Scene scene = SceneManager.GetSceneByPath(sceneFilePath);
            if (scene.IsValid())
            {
#if UNITY_EDITOR
